...
function renderList(files: FileItem[], selectedIndex: number) {
  ...
  console.log(
    chalk.gray(
      "Use ↑/↓ to navigate, SPACE to toggle selection, ENTER to delete, ESC to exit\n",
    ),
  );
  ...
  console.log(
    chalk.gray(
      `[${allSelected ? "x" : noneSelected ? " " : "-"}] Select All / Deselect All`,
    ),
  );
  console.log("");
  ...
  console.log(
    chalk.gray(`Selected: ${selectedCount} file(s) | Press ENTER to delete`),
  );
}
...
program
  .name("codeprune")
  .description(
    "A CLI tool to detect and clean up unused files and imports in JS/TS projects",
  )
  .version("1.0.0");
...
program
  ...
  .description(
    "Initialize codeprune.config.json (framework: next, react, react-native, node, vue, svelte, express)",
  )
  ...
  .action(async (framework, options) => {
    ...
    if (!f) {
      if (detected && options.yes) {
        f = detected;
        console.log(
          chalk.cyan(`🔍 Detected framework: ${chalk.bold(detected)}`),
        );
      } else if (detected) {
        console.log(
          chalk.cyan(`🔍 Detected framework: ${chalk.bold(detected)}`),
        ...
        console.log(
          chalk.gray(
            `  Press Enter to use this or type a different framework.\n`,
          ),
        );
        ...
        await new Promise<void>((resolve) => {
          rl.question(
            chalk.gray(`Select framework (${frameworks.join(", ")}): `),
            (answer) => {
              rl.close();
              if (answer.trim()) {
        ...
        if (options.yes) {
          f = "node";
          console.log(
            chalk.yellow("⚠️ Could not detect framework, using default: node"),
          );
        } else {
          console.log(
            chalk.yellow(
              `Could not auto-detect framework. Please specify one:`,
            ),
          );
          console.log(
            chalk.gray(`Usage: codeprune init <${frameworks.join("|")}>`),
          );
          return;
        }
      }
  ...
  .action(async (options) => {
    ...
    if (options.all) {
      const files = fs
        ...
        .filter((f) => f !== ".codeprune-manifest.json");
      if (files.length === 0) {
      ...
      console.log(
        chalk.gray(
          'Use: codeprune restore --file "path/to/file" to restore specific',
        ),
      );
    }
  });
...
program
  ...
  .option(
    "-f, --fix-imports",
    "remove unused imports, variables, functions, classes and organize imports",
  )
  .action(async (options) => {
    const cwd = process.cwd();
    ...
    if (isJson) {
      const output: Record<string, unknown> = {
        ...
        possiblyUnused: result.possiblyUnusedFiles.map((f) =>
          path.relative(cwd, f),
        ),
      };
      ...
      if (result.possiblyUnusedFiles.length > 0) {
        ...
        console.log(
          chalk.yellow(
            `⚠️ Possibly Unused (dynamic import detected) (${result.possiblyUnusedFiles.length}):`,
          ),
        );
      ...
      console.log(
        chalk.green(
          `✅ Done in ${timeTaken}s (Scanned ${result.totalFiles} files)`,
        ),
      );
      ...
      if (options.fixImports && files.length > 0) {
        ...
        for (const file of files) {
          ...
          if (
            ...
            (analysis.unusedImports.length > 0 ||
              analysis.unusedDeclarations.length > 0)
          ) {
        ...
        if (filesWithIssues.length > 0) {
          ...
          console.log(
            chalk.gray(
              `  Total: ${totalUnusedImports} unused import(s), ${totalUnusedDeclarations} unused declaration(s)`,
            ),
          );
        }
        ...
        if (fixedCount > 0 || removedImports > 0 || removedDeclarations > 0) {
          ...
          if (removedImports > 0) {
            console.log(
              chalk.yellow(`🗑️  Removed ${removedImports} unused import(s)`),
            );
          }
          if (removedDeclarations > 0) {
            console.log(
              chalk.yellow(
                `🗑️  Removed ${removedDeclarations} unused declaration(s)`,
              ),
            );
          }
        } else {
      ...
      if (options.delete && result.unusedFiles.length > 0) {
        ...
        if (options.interactive) {
          ...
          console.log(
            chalk.cyan(
              `🗑️  Moving ${toDelete.length} file(s) to .codeprune-trash...`,
            ),
          );
          const trashDir = path.resolve(cwd, ".codeprune-trash");
          const manifestPath = path.resolve(
            trashDir,
            ".codeprune-manifest.json",
          );
          if (!fs.existsSync(trashDir)) {
          ...
          for (const file of toDelete) {
            ...
            try {
              ...
              console.error(
                chalk.red(`Failed to move ${file.path}: ${e.message}`),
              );
            }
          }
          ...
          console.log(
            chalk.green(
              "✅ Cleanup complete! (Check .codeprune-trash to recover if needed)",
            ),
          );
        } else {
          console.log("");
          console.log(
            chalk.cyan("🗑️  Moving unused files to .codeprune-trash..."),
          );
          const trashDir = path.resolve(cwd, ".codeprune-trash");
          const manifestPath = path.resolve(
            trashDir,
          ...
          for (const file of result.unusedFiles) {
            ...
            try {
              ...
              console.error(
                chalk.red(`Failed to move ${relativePath}: ${e.message}`),
              );
            }
          }
